...
class BaseModelBasedEnv(gym.Env, abc.ABC):
    ...
    # New methods
    def reset(self, runner_type: str):
        if runner_type == 'real':
            return self.real_reset()
        elif runner_type == 'virtual':
            return self.virtual_reset()
        else:
            raise ValueError('(BaseModelBasedEnv) `runner_type` provided is not valid. If you were not expecting this '
                             'error you are not probably using SPlaceEnv.')
