...
from slbo.envs.splace import utils
...
class FLAGS(BaseFLAGS):
    ...
    def set_seed(cls):
        ...
        logger.warning("Setting GLOBAL (and numpy) seed to %s", cls.seed)
        ...
        tf_seed = np.random.randint(2**30)
        tf.set_random_seed(tf_seed)

        cpython_seed = np.random.randint(2**30)
        random.seed(cpython_seed)
        utils.set_seed(cpython_seed)

        logger.warning('Setting generated tf seed to %d' % tf_seed)
        logger.warning('Setting generated cpython seed to %d' % cpython_seed)
...
if FLAGS.env.expert:
    FLAGS.env.reproducibility = False
    logger.warning('Since `expert` is set to True, the standard reservations will not be loaded`')
