...
def main():
    ...
    if utils.expert:
        print()
        logger.warning('EXPERT ENVIRONMENT ACTIVATED\n')
    ...
    runners = {
        'test': make_real_runner(FLAGS.test_runner_n_envs),
        'collect': make_real_runner(FLAGS.collect_runner_n_envs),
        'dev': make_real_runner(FLAGS.dev_runner_n_envs),
    ...
    for T in range(FLAGS.slbo.n_stages):
        logger.info('------ Starting Stage/Day %d --------', T + 1)
        if utils.evaluate:
        ...
        if utils.reproducibility:
            assert T == runners['collect'].env.envs[0].day
        ...
        if utils.n_outside_action_range > 0:
            logger.info('(SafePlaceEnv) Times outside action range: %d' % utils.n_outside_action_range)
            utils.reset_action_range_stat()
