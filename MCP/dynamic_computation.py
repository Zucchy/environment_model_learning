...
def uct(initial_state) -> None:
    """
    Main function that executes a single run of MCTS algorithm (Plain UCT variant).
...
if __name__ == '__main__':
    # Single run of MCTS (program different from all the other variants)

    # Best parameters found so far (with `utils.energy_factor` = 0.1 and `max_depth` = 13)
    exp_const = 10
    max_depth = 13
    rollout_moves = 0
    ...
    # Environment that uses the neural network through PyPy interpreter
    if utils.adaptability_nn_pypy:
        # Set the path of your model weights
        env = SafePlaceEnvNNPyPy('nn/pretrain/pickle/'
                                 'safeplace_simulated_nn_(15, 30, 40)_1000_220805_163956_tf_seed_3_val_loss_0_04.pickle')

    # Environment that uses the neural network through the standard implementation of TensorFlow
    elif utils.adaptability_nn_tf:
        # Set the path of your model
        nn = NN(model_path='safeplace_simulated_nn_(15, 30, 40)_1000_220805_163956.h5')
