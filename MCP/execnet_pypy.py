...
def uct(initial_state: StateData) -> (list, float, list, list, list):
    """
    This function executes a single run of MCTS algorithm (Plain UCT variant). It saves transition statistics if
    `utils.step_stats` is set to True and saves prediction error statistics if `utils.batch_stats` is set to True.

    Args:
        initial_state: initial `StateData` object of the MDP.

    Returns: i) transition data, ii) cumulative reward of the day, iii) list of CO2 prediction absolute errors, iv) list
    of VOCs prediction absolute errors and v) list of indoor temperature prediction absolute errors.
    """
...
def oracle_uct(initial_state: StateData) -> float:
    """
    This function executes a single run of MCTS algorithm (Plain UCT variant) with oracle's transition model. It saves
    transition statistics if `utils.step_stats` is set to True.

    Args:
        initial_state: initial `StateData` object of the MDP.

    Returns: the cumulative reward of the day following the oracle policy.
    """
    ...
    while not root.state.is_terminal:
        ...
        # A new tree must be created in order to compare fairly with `uct` function
        root = real_sn
...
def mcts(channel, _batches, _simulations, _datasets, _timestamp) -> None:
    """
    One of the two main functions used in our experiments (the other one is `train()` in `execnet_tf.py`). It is
    executed by PyPy interpreter if `utils.execnet_interpreter_path` is set correctly. This significantly speeds up
    the computation time.

    The function loads the room reservations profile if `utils.reproducibility` is set to True; otherwise, it generates
    a random profile at each iteration of the main loop.

    For the number of times set in the `_batches` variable, the main loop executes the following operations:
        - waits to receive the location in the file system of the serialized `NNPyPy` object;
        - loads the neural network weights;
        - for the number of times set in the `_simulations` variable (i.e., number of days, always set to 1 in our
            experiments), a new loop executes the following operations:
            1. loads or creates the room reservations profile;
            2. initializes the MCTS framework;
            3. performs a complete MCTS simulation (i.e., until it reaches a terminal state) using the variant related
                neural network as transition model while receiving real transitions from the real environment;
            4. saves statistics of different types (oracle statistics if `utils.compare_with_oracle` is set to True,
                prediction error statistics if `utils.batch_stats` is set to True);
            5. shuffles the new transition dataset generated by the new observations of the real environment and saves
                them in the file system (the location is given by `_datasets` variable) in a csv file.
        - all the new data obtained in the previous loop is sent through the channel to the CPython process (thanks to
            this expedient, the CPython process never loads the dataset of the real observations from the file system.
            However, the data is also saved in a csv file to allow us to view it afterwards).

    All these operations happen independently of the active variant.
    """

    # MCTS parameters used in our experiments
    exp_const = 10
    max_depth = 13
    rollout_moves = 0
    ...
    if utils.reproducibility:
        ...
        if not utils.all_rooms:
            ...
            reproducibility_reservations = reproducibility_reservations[:_batches]
        ...
        if not utils.all_rooms:
            ...
            reproducibility_temp_in = reproducibility_temp_in_temp[:_batches]
    ...
    if utils.batch_stats:
        ...
        if utils.use_two_neural_networks:
            filepath = folder + 'safeplace_additional_stats_' + _timestamp + '.pickle'
            if utils.verbose:
                execnet_print('(mcts) Saving additional statistics in the following file: ' + filepath, flush=True)
            with open(filepath, 'wb') as f:
                pickle.dump(env.additional_stats, f)
...
if __name__ == '__channelexec__':
    """
    Here starts the execution of the PyPy process.
    """
    import os
    os.dup2(2, 1)
    ...
    def execnet_print(*args, **kwargs) -> None:
        """
        Function used to print messages to terminal with a different color when using `execnet` module with a PyPy
        created process. It works exactly as the standard `print()` function.

        Args:
            *args: parameters of the subclass `execnet_print` method. Then passed to `print()` function.
            **kwargs: additional parameters of the subclass `do_transition` method in the form of a dict-like structure.
                Then passed to `print()` function.
        """
        new_args = []
        length = len(args)
        if length != 0:
