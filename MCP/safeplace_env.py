...
class SafePlaceEnv(Environment):
    ...
    def get_prediction_error(s: StateData, a: ActionData) -> list[float, float, float]:
        """
        Compares the predictions of this model with the oracle's model ones and returns them as a list. In this case
        this class would compare its predictions with themselves. Therefore, the function returns [0, 0, 0].

        Args:
            s: `StateData` object representing the current state.
            a: `ActionData` object representing the action.

        Returns: a list of zeros: [0, 0, 0].
        """
        return [0, 0, 0]
